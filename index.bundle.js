...
#app {
  position: relative; /* containing block for the .weatherBg drift layer */
  min-height: 100dvh;
  display: flex;
...
/* Drifting-gradient background: two overlapping soft radial glows that slowly
   move and breathe behind the app, mounted by sky.js as the first child of #app.
   \`.weatherBg\` clips its own glow (overflow: hidden) so the drift stays inside
   the app column — the clip is scoped here rather than on #app so it can't clip
   the #suggestions dropdown or the mobile #weatherCard slide animation. It sits
   under the content (z-index: 0 vs the content's z-index: 1). Colours are the
   cool blue/teal of the temperature ramp at low opacity so the glow reads
   without competing with the readout. */
.weatherBg {
  position: absolute;
  inset: 0;
  z-index: 0;
...
.weatherBg::before {
  top: -25%;
  left: -20%;
  background: radial-gradient(circle at center, rgba(61, 111, 168, 0.22), rgba(61, 111, 168, 0) 68%);
...
.weatherBg::after {
  bottom: -25%;
  right: -20%;
  background: radial-gradient(circle at center, rgba(111, 168, 160, 0.18), rgba(111, 168, 160, 0) 68%);
  animation: bgDriftB 32s ease-in-out infinite alternate;
}

@keyframes bgDriftA {
  0%   { transform: translate(0, 0) scale(1);       opacity: 0.65; }
  100% { transform: translate(18%, 14%) scale(1.15); opacity: 1; }
}

@keyframes bgDriftB {
  0%   { transform: translate(0, 0) scale(1.1);      opacity: 1; }
  100% { transform: translate(-16%, -12%) scale(1);  opacity: 0.6; }
}
...
/* Halt the drift entirely under a reduced-motion preference — one static frame. */
