...
**Files to fill in:**
- [ ] Replace `WeatherApp_TOP` everywhere in this file with this project's name (the repo name is usually fine: e.g. `my-app`, `react-thing`, `cool-project`).
...
- [ ] Replace `WeatherApp_main/src/` with the path from the repo root to the source directory (e.g. `src/` for most projects, `app/src/` if nested, `packages/web/src/` for monorepos).
...
- [ ] Fill in the "Key files in this repo" section with the load-bearing files in `WeatherApp_main/src/` and brief descriptions of each.
...
- [ ] Add a target to the `todo-injector-worker` repo's `ALLOWED_TARGETS` in `src/index.js`:
  ```
  { repo: "<owner>/<repo>", filePath: "TODO.md", srcPrefix: "WeatherApp_main/src/" },
  ```
- [ ] Deploy the worker: `cd todo-injector-worker && npm run deploy`.
- [ ] In the PWA's chat surface, switch the workspace pill to this repo and verify the file manifest loads (the picker should list files from `WeatherApp_main/src/`).
...
**Project name:** WeatherApp_TOP

**Description:** (fill in a one-line description)

**Stack:** CommonJS

**Source directory:** `WeatherApp_main/src/`
...
>
> - `WeatherApp_main/src//main.js` (or equivalent entry point) — what it does
> - `WeatherApp_main/src//dataLayer.js` — data model / state — note if ALL mutations route through here
> - `WeatherApp_main/src//components/Foo.jsx` — what it does
...
- [ ] **[PRIORITY]** Imperative verb + specific change
  ...
  - File: `WeatherApp_main/src//main.js`, `WeatherApp_main/src//style.css`
